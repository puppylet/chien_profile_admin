...
import {clients} from '../../../store/models'
...
export default class CreateClient extends Component {
  ...
  _handleSubmit = (back) => {
    ...
    const {logo, name, website, description} = this.state
    const data = {name, website, description}
    if (logo.file) data.logo = logo.preview

    post(clients, data).then(res => {
      if (back) this._goBack()
      else history.push('/clients/details/' + res._id)
      toast.success('Create client Success')
    }).catch(err => {
        ...
        <Segment.Group>
          <Segment color='blue' className='segment-header'>
            <div className='clearfix'>
              <h2 className='left'>Add client</h2>
            </div>
          </Segment>
          <Segment>
