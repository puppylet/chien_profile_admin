...
class Pagination extends Component {
  _jumpToPage = (e, {value}) => {
    const {history, url, onPageChange} = this.props
    if (onPageChange) return onPageChange(value)
      ...
      <div className='clearfix ms-pagination'>
        ...
        {totalPages < 2 ? null : [
          <Menu floated='right' pagination size='tiny' key={1}>
            <Menu.Item as={Link} icon to={url} disabled={currentPage === 1}>
              <Icon name='angle double left' />
            </Menu.Item>
            <Menu.Item as={Link} icon to={currentPage <= 2 ? url : `${url}/${currentPage - 1}`} disabled={currentPage === 1}>
            ...
            <Menu.Item as={Link} icon to={currentPage === totalPages ? `${url}/${totalPages}` : `${url}/${currentPage + 1}`} disabled={currentPage === totalPages}>
              <Icon name='angle right' />
            </Menu.Item>

            <Menu.Item as={Link} icon to={`${url}/${totalPages}`} disabled={currentPage === totalPages}>
